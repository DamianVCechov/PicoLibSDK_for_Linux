...
#if USE_ST7365P		// use ST7365P TFT display (st7365p.c, st7365p.h)
...
#include "../../_devices/clockwork/clockwork_ss.h"
#include "../../_lib/inc/lib_text.h"
#include "../../_lib/inc/lib_draw.h"

// ST7365P commands
#define ST7365P_NOP		0x00	// no operation
#define ST7365P_SWRESET		0x01	// software reset
#define ST7365P_RDDID		0x04
#define ST7365P_RDDST		0x09

#define ST7365P_SLPIN		0x10	// enable sleep mode
#define ST7365P_SLPOUT		0x11	// disable sleep mode
#define ST7365P_PTLON		0x12
#define ST7365P_NORON		0x13

#define ST7365P_INVOFF		0x20	// disable inversion mode
#define ST7365P_INVON		0x21	// enable inversion mode
#define ST7365P_DISPOFF		0x28	// disable display
#define ST7365P_DISPON		0x29	// enable display
#define ST7365P_CASET		0x2A	// set start and end column of draw window
#define ST7365P_RASET		0x2B	// set start and end row of draw window
#define ST7365P_RAMWR		0x2C	// start write data to draw window
#define ST7365P_RAMRD		0x2E	// start read data from draw window
...
#define ST7365P_MADCTL_MV	0x20	// B5: page/column order (0=normal mode, 1=reverse mode)
#define ST7365P_MADCTL_ML	0x10	// B4: line address order (LCD refresh 0=top to bottom, 1=bottom to top)
#define ST7365P_MADCTL_RGB	0x08	// B3: RGB/BGR order (0=RGB, 1=BGR)
#define ST7365P_MADCTL_MH	0x04	// B2: display latch order (LD refresh 0=left to right, 1=right to left)

#define ST7365P_RDID1		0xDA
#define ST7365P_RDID2		0xDB
#define ST7365P_RDID3		0xDC
#define ST7365P_RDID4		0xDD

// color mode ST7365P_COLMOD (select both color space and data format)
...
// rotation mode for ST7365P_MADCTL
// Matematicky vypočítané orientace pro displej s výchozím nastavením 0x48
const u8 RotationTab[4] = {
	0x48,		// 0: Portrait (MX=1, RGB=1)
	0x28,		// 1: Landscape (MV=1, RGB=1)
	0x88,		// 2: Inverted Portrait (MY=1, RGB=1)
	0xE8,       // 3: Inverted Landscape (MY=1, MX=1, MV=1, RGB=1)
};

u8 DispRot = 0;			// current display rotation 0..3
...
{
	DispWriteCmd(ST7365P_SLPIN);
}

// disable sleep mode
void DispSleepDisable()
{
	DispWriteCmd(ST7365P_SLPOUT);
}

// enable inversion mode
void DispInvEnable()
{
	DispWriteCmd(ST7365P_INVON);
}

// disable inversion mode
void DispInvDisable()
{
	DispWriteCmd(ST7365P_INVOFF);
}

// set color mode COLOR_MODE_*
void DispColorMode(u8 mode)
{
	DispWriteCmdData(ST7365P_COLMOD, &mode, 1);
}
...
{
	DispRot = rot;
	DispWriteCmdData(ST7365P_MADCTL, &RotationTab[rot], 1);
}	
...
{
	u16 buf[2];
	u16 off_x = DISP_OFFSET_X;
	u16 off_y = DISP_OFFSET_Y;

	u8 madctl = RotationTab[DispRot];

	// Dynamický výpočet offsetu: Čip má fyzickou paměť 320x480, displej je 320x320.
	// Hledáme, na které fyzické ose je oněch 160 pixelů mimo obrazovku a případně o ně okno posuneme.
	if ((madctl & ST7365P_MADCTL_MV) == 0) {
		// Portrait orientace (MV=0)
		// CASET (x) = fyzická osa 320px, RASET (y) = fyzická osa 480px
		if (madctl & ST7365P_MADCTL_MX) off_x += (320 - 320); // 0
		if (madctl & ST7365P_MADCTL_MY) off_y += (480 - 320); // 160
	} else {
		// Landscape orientace (MV=1)
		// CASET (x) dostává data pro fyzickou osu 480px, RASET (y) pro osu 320px
		if (madctl & ST7365P_MADCTL_MX) off_x += (480 - 320); // 160
		if (madctl & ST7365P_MADCTL_MY) off_y += (320 - 320); // 0
	}

	// set columns
	buf[0] = ENDIAN16(x1 + off_x);
	buf[1] = ENDIAN16(x2 - 1 + off_x);
	DispWriteCmdData(ST7365P_CASET, buf, 4);

	// set rows
	buf[0] = ENDIAN16(y1 + off_y);
	buf[1] = ENDIAN16(y2 - 1 + off_y);
	DispWriteCmdData(ST7365P_RASET, buf, 4);

	// send command to send data
	DispWriteCmd(ST7365P_RAMWR);
}
...
void DispSendImg2(u16 data)
{
	...
	// Řešení Endianity: ST7365P očekává jako první High bajt. 
	// Proto posíláme nejprve MSB a poté LSB.
	while (SPI_TxIsFull(DISP_SPI)) {}
	SPI_Write(DISP_SPI, (u8)(data >> 8)); // Odeslání High Bajtu (MSB)
	SPI_RxFlush(DISP_SPI);

	while (SPI_TxIsFull(DISP_SPI)) {}
	SPI_Write(DISP_SPI, (u8)data);        // Odeslání Low Bajtu (LSB)
	SPI_RxFlush(DISP_SPI);
}
...
void DispUpdate()
{
	if ((DispDirtyX1 < DispDirtyX2) && (DispDirtyY1 < DispDirtyY2))
	{
		...
		u16 line_buf[WIDTH]; // Řádkový buffer pro softwarovou záměnu endianity (max 320*2 = 640 B)

		for (i = DispDirtyY2 - DispDirtyY1; i > 0; i--)
		{
			// Softwarové přehození bajtů pro ST7365P, který vyžaduje Big-Endian.
			// Na RP2350 je tento kopírovací cyklus extrémně rychlý.
			for (j = 0; j < w; j++) {
				u16 c = s0[j];
				line_buf[j] = (c >> 8) | (c << 8); 
			}
			...
			if (w > 20)
			{
				DispWriteDataDMA(line_buf, w*2);
			}
			else
#endif // USE_DISP_DMA
			{
				DispWriteData(line_buf, w*2);
			}
			s0 += WIDTH;
		}
...
#endif // USE_ST7365P		// use ST7365P TFT display (st7365p.c, st7365p.h)
